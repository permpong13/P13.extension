...
__doc__ = "Numbering + Preview + Predicted Preview (Revit 2025, pyRevit)"
...
# get doc/uidoc (must run inside Revit)
try:
...
BUILTIN_MAP = {
    ...
    "Specialty Equipment": getattr(BuiltInCategory, "OST_SpecialityEquipment", None),
    ...
    "Detail Items": getattr(BuiltInCategory, "OST_DetailItems", None),
...
class NumberingForm(Form):
    def __init__(self):
        ...
        self.active_view = doc.ActiveView  # Get current active view
    ...
    def _create_right_panel(self):
        ...
        # Current View Info
        view_info = Label()
        view_info.Text = "วิวปัจจุบัน: " + (self.active_view.Name if self.active_view else "ไม่มี")
    ...
    def _highlight_zoom(self, s, e):
        try:
            ...
            for el in self.preview_elements:
                ids.Add(el.Id)
            uidoc.Selection.SetElementIds(ids)  # select
    ...
    def _step(self, s, e):
        try:
            ...
            try:
                ...
                row.Cells["Numbered"].Value = "ใช่"
                self._set_status("ขั้นตอนองค์ประกอบ {} = {} ({} หลัก)".format(el.Id.IntegerValue, value, digit_count))
            finally:
                t.Commit()
        except Exception as ex:
