...
__doc__ = "DiRoots 3-Pane UI + Single Window + Auto Save Settings + Unchecked Family (Revit 2026, pyRevit)"
...
def get_element_category(el):
    try:
        if el and el.Category: return el.Category.Name
    except: pass
    return "Unknown"
...
def filter_elements_by_view_visibility(elements, active_view):
    ...
    return [el for el in elements if not (hasattr(el, 'IsHidden') and active_view.IsElementHidden(el.Id))]
...
class NumberingForm(Form):
    ...
    def _style_datagrid(self, grid):
        ...
        grid.RowHeadersVisible = False # Flat look (ซ่อนหัวแถว)
    ...
    def _build_ui(self):
        # ---------------- 1. LEFT PANE: Selection (Width: 380) ----------------
        ...
        c_name = DataGridViewTextBoxColumn(HeaderText="หมวดหมู่", Name="Category", ReadOnly=True, Width=120)
        ...
        self._create_label("FAMILY TYPES", Point(15, 350), Size(300, 20), self.Font, pnl_left, True)
        ...
        # ---------------- 2. CENTER PANE: Preview Grid (Width: 560) ----------------
        pnl_center = Panel(Location=Point(400, 10), Size=Size(560, 700), BackColor=Color.White, BorderStyle=BorderStyle.FixedSingle)
        
        self._create_label("ELEMENTS PREVIEW", Point(15, 10), Size(300, 20), self.Font, pnl_center, True)
        ...
        # ---------------- 3. RIGHT PANE: Properties & Rules (Width: 260) ----------------
        ...
        # Format
        self._create_label("NUMBERING FORMAT:", Point(15, 200), Size(200, 20), self.Font, pnl_right, True)
        self._create_label("Start (เริ่ม):", Point(15, 230), Size(70, 20), self.Font, pnl_right)
        self.txt_start = TextBox(Text=self.config.get("StartNumber", "1"), Location=Point(85, 227), Size=Size(150, 24), Font=self.Font)
        pnl_right.Controls.Add(self.txt_start)
        self._create_label("Step (+):", Point(15, 265), Size(70, 20), self.Font, pnl_right)
        self.txt_step = TextBox(Text=self.config.get("StepValue", "1"), Location=Point(85, 262), Size=Size(150, 24), Font=self.Font)
        pnl_right.Controls.Add(self.txt_step)
        
        self._create_label("Digits (หลัก):", Point(15, 305), Size(80, 20), self.Font, pnl_right)
        self.digit_radios = []
        for i, digits in enumerate(["1", "2", "3", "4"]):
            rb = RadioButton(Text=digits, Location=Point(95 + (i*35), 303), Size=Size(35, 20), Tag=int(digits), Font=self.Font)
            if str(digits) == str(self.config.get("Digits", "2")): rb.Checked = True
            self.digit_radios.append(rb)
            pnl_right.Controls.Add(rb)

        # Direction
        self._create_label("SORTING DIRECTION:", Point(15, 360), Size(200, 20), self.Font, pnl_right, True)
        ...
        self._create_label("MANUAL OVERRIDE:", Point(15, y+20), Size(200, 20), self.Font, pnl_right, True)
        self.rb_manual_pick = RadioButton(Text="คลิกเลือกเองใน 3D", Location=Point(15, y+45), Size=Size(220, 20), Font=Font("Segoe UI", 9, FontStyle.Bold), ForeColor=Color.FromArgb(192, 57, 43))
        pnl_right.Controls.Add(self.rb_manual_pick)
        ...
        self.btn_load = self._create_btn("↻ โหลดข้อมูล (Load Preview)", Point(0, 0), Size(200, 35), self._load_preview, Color.FromArgb(41, 128, 185))
        self.btn_hl = self._create_btn("👁 ค้นหาในโมเดล", Point(210, 0), Size(120, 35), self._highlight_zoom, Color.FromArgb(149, 165, 166))
        self.btn_exp = self._create_btn("⬇ Export CSV", Point(340, 0), Size(110, 35), self._export_csv, Color.FromArgb(22, 160, 133))
        self.btn_imp = self._create_btn("⬆ Import CSV", Point(460, 0), Size(110, 35), self._import_csv, Color.FromArgb(39, 174, 96))
        
        self.lbl_status = self._create_label("Status: Ready", Point(590, 8), Size(350, 20), self.Font, pnl_bot, False, Color.Teal)
        
        self.btn_run = self._create_btn("▶ ดำเนินการ (Apply Numbering)", Point(960, 0), Size(250, 35), self._run_all, Color.FromArgb(39, 174, 96)) 
    ...
    def _on_category_selected(self, s, e):
        ...
        for sub_name, count in sorted(subcats.items()):
            # กำหนดให้ค่าเริ่มต้นเป็น False (ไม่เช็คถูกอัตโนมัติ)
    ...
    def _load_preview(self, s, e):
        self.preview_elements = []
        is_manual = self.rb_manual_pick.Checked
        
        if is_manual:
            self.Hide()
            try:
                sel_refs = uidoc.Selection.PickObjects(ObjectType.Element, "คลิกเลือกวัตถุตามลำดับ (กด ESC เพื่อจบ)")
                for ref in sel_refs:
                    el = doc.GetElement(ref)
                    cat_name = get_element_category(el)
                    pref, suf = "", ""
                    for r in self.grid_cat.Rows:
                        if r.Cells["Category"].Value == cat_name:
    ...
    def _populate_preview_grid(self):
        ...
        param_name = self.txt_custom.Text if self.chk_custom.Checked else self.cmb_param.SelectedItem
        
        for el, cat, pref, suf in self.preview_elements:
            final_val = "{}{}{}".format(pref, str(current_num).zfill(digit_count), suf)
            current_val = ""
            try:
                p = el.LookupParameter(param_name)
                if p: current_val = p.AsString() or ""
            except: pass
    ...
    def _save_settings_on_close(self, sender, e):
        cats = {}
        for r in self.grid_cat.Rows:
...
def main():
    try:
        if doc: NumberingForm().ShowDialog()
    except Exception as ex:
