...
__doc__ = "สคริปต์นี้จะช่วยให้ผู้ใช้สามารถเลือกพารามิเตอร์ที่ต้องการแสดงค่า แล้วนำมาแสดงเป็นข้อความกำกับไว้ใกล้กับองค์ประกอบในตารางคำอธิบายสัญลักษณ์ โดยสามารถเลือกตำแหน่งการวางข้อความและรูปแบบตัวอักษรได้ตามต้องการ"
...
def get_param_value_as_string(param):
    """Get parameter value as string with proper formatting"""
    ...
    try:
        ...
        if storage_type == DB.StorageType.Double:
            ...
            # For unit conversions, you might need additional logic
            ...
            return element.Name if element else "Invalid"
        else:
            # Use AsValueString as fallback
            return param.AsValueString() or "N/A"
    except:
        return "Error"
    ...
    for l, ton in izip(legend_components, types_on_legend):
        ...
        for sp in selected_parameters:
            try:
                ...
                param_value = get_param_value_as_string(p)
                if param_value and param_value != "N/A":
                    if show_p_name:
