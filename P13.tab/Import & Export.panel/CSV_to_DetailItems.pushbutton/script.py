...
__doc__ = "อ่านพิกัดจากไฟล์ CSV แล้วสร้าง Detail Items ใน Plan View โดยอัตโนมัติ (รองรับ Revit 2026)"
...
def get_project_base_point_coordinates():
    """ดึงค่าพิกัดจาก Project Base Point (ปรับปรุงสำหรับ Revit 2026)"""
    try:
        # วิธีหลักสำหรับ Revit เวอร์ชันใหม่ (ใช้ Collector)
        ...
        for bp in collector:
            if not bp.IsShared:  # Project Base Point
                ...
                if east_param and east_param.HasValue and north_param and north_param.HasValue:
                    ...
                    print("✅ ดึงค่าพิกัดจาก Project Base Point สำเร็จ (Collector):")
        ...
        # วิธีสำรอง: ใช้ ProjectLocation
        try:
            project_location = doc.ActiveProjectLocation
            if project_location:
                # GetProjectPosition method might need verification in 2026 API but usually stable
...
def get_actual_project_base_point_position():
    ...
    try:
        ...
        for bp in collector:
            if not bp.IsShared:
                # ใน Revit 2026 บางครั้งต้องใช้ Position property หรือ BoundingBox
                # Position property ยังคงมีอยู่แต่มักจะเป็น Read-only
...
class BasePointInputForm(Form):
    ...
    def InitializeComponent(self):
        self.Text = "ตรวจสอบค่า Base Point (2026)"
...
def select_csv_file():
    ...
    # ใช้งาน Last Folder Memory
    last_folder = load_last_folder()
    if last_folder:
        dialog.InitialDirectory = last_folder
    
    if dialog.ShowDialog() == DialogResult.OK:
        save_last_folder(dialog.FileName) # บันทึก path ล่าสุด
...
def get_all_detail_family_symbols(doc):
    ...
    try:
        ...
        # เพิ่ม Logic สำหรับค้นหา Family ที่ยังไม่มี Instance (แต่โหลดเข้ามาแล้ว)
...
def create_custom_detail_item(view, location, pile_no, e, n):
    try:
        # ใช้ Plane.Create (Revit 2026 Compatible)
        radius = 0.5
        center = location
        normal = XYZ.BasisZ
        
        # แก้ไข: Plane.Create แทน CreateByNormalAndOrigin ที่ถูกยกเลิก
        ...
        for i in range(4):
            ...
            # แก้ไข: ใช้ Signature ของ Arc.Create ที่แม่นยำกว่าสำหรับ 2025/2026
