...
class SuperSheetsUltimate(Window):
    def __init__(self):
        self.Title = "P13 SuperSheets Pro Max 2026 | Dual Export (PDF & DWG)"
    ...
    def _setup_ui(self):
        ...
        # 1. PROFILE & FORMAT (Updated to Multi-Select)
        ...
        self.chkPDF = CheckBox(); self.chkPDF.Content = "PDF"; self.chkPDF.IsChecked = True; self.chkPDF.VerticalAlignment = VerticalAlignment.Center
        self.chkDWG = CheckBox(); self.chkDWG.Content = "DWG"; self.chkDWG.Margin = Thickness(10,0,0,0); self.chkDWG.VerticalAlignment = VerticalAlignment.Center
        sp_prof.Children.Add(self.chkPDF); sp_prof.Children.Add(self.chkDWG)
        
        sp_prof.Children.Add(TextBlock(Text=" | PDF Mode: ", VerticalAlignment=VerticalAlignment.Center, Margin=Thickness(15,0,0,0)))
        ...
        self.spCombineName.Children.Add(TextBlock(Text="Combined PDF Filename:", FontWeight=FontWeights.Bold))
        ...
        sp_f.Children.Add(TextBlock(Text="Search: ", VerticalAlignment=VerticalAlignment.Center, FontWeight=FontWeights.Bold))
        ...
        self.dg.Columns.Add(DataGridCheckBoxColumn(Header="Export", Binding=Binding("Include")))
        ...
        # 5. START
        btn_run = Button(); btn_run.Content = " START MULTI-EXPORT "; btn_run.Height = 45; btn_run.Margin = Thickness(0,10,0,0); btn_run.Background = Brushes.ForestGreen; btn_run.Foreground = Brushes.White; btn_run.FontWeight = FontWeights.Bold; btn_run.Click += self._on_export
    ...
    def _save_current_as_last(self):
        data = {
            ...
            'chk_pdf': self.chkPDF.IsChecked, 'chk_dwg': self.chkDWG.IsChecked,
        ...
        if os.path.exists(LAST_SETTING_FILE):
            try:
                with open(LAST_SETTING_FILE, 'r') as f:
                    ...
                    self.chkPDF.IsChecked = p.get('chk_pdf', True); self.chkDWG.IsChecked = p.get('chk_dwg', False)
    ...
    def _on_export(self, s, e):
        ...
        if not self.chkPDF.IsChecked and not self.chkDWG.IsChecked:
            forms.alert("Please select at least one Format (PDF or DWG)!"); return
        ...
        do_dwg = self.chkDWG.IsChecked
        combine_pdf = self.chkCombine.IsChecked if do_pdf else False

        try:
            total_tasks = len(selected) * (int(do_pdf and not combine_pdf) + int(do_dwg)) + (1 if combine_pdf else 0)
            
            with forms.ProgressBar(title="P13 Multi-Exporting...", total=total_tasks) as pb:
                task_count = 0
                
                # 1. Handle Combined PDF first (if applicable)
                if combine_pdf:
                    ...
                    task_count += 1; pb.update_progress(task_count, total_tasks)

                # 2. Handle Individual Exports (PDF/DWG)
                for item in selected:
                    ...
                    if do_dwg:
                        ...
                        doc.Export(folder, fn, List[DB.ElementId]([item.Id]), opt_dwg)
                        task_count += 1; pb.update_progress(task_count, total_tasks)

            forms.alert("Multi-Format Export Successful!"); self.Close()
