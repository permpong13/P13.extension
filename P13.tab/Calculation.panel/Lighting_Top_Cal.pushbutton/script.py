...
# ⭐️ แก้ไขให้ Base_Level ถูกสร้าง/ตรวจสอบเป็นชนิด "Length"
status_base = setup_parameter(doc, app, "Base_Level", "Length", cat_lighting)
...
if status_base == "created": output.print_md("✅ **Base_Level** (Length) ถูกสร้างอัตโนมัติ")
...
with forms.ProgressBar(title='กำลังคำนวณพารามิเตอร์ดวงโคม... ({value} จาก {max_value})', cancellable=True) as pb:
    for index, item in enumerate(fixtures):
        ...
        try:
            ...
            # ⭐️ (A) Base_Level: ปรับให้รองรับการเขียนค่าตัวเลข (Double) ลงในพารามิเตอร์ประเภท Length โดยตรง
            ...
            if p_base and not p_base.IsReadOnly:
                ...
                if is_in_group and not varies_across_groups:
                    ...
                    if p_base.StorageType == DB.StorageType.Double:
